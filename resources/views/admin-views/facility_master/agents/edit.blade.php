...
@section('title', ui_change('edit_agent' , 'property_master') )
@php
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2">
                ...
                {{ ui_change('edit_agent' , 'property_master')  }}
            </h2>
        </div>
        <!-- End Page Title -->

        <!-- Form -->
        <form class="product-form text-start" action="{{ route('agent.update', $agent->id) }}" method="POST"
                ...
                <div class="card-header">
                    <div class="d-flex gap-2">
                        {{-- <img src="{{ asset(main_path() . 'back-end/img/shop-information.png') }}" class="mb-1"
                            alt=""> --}}
                        <h4 class="mb-0">{{ ui_change('edit_agent' , 'property_master')  }}</h4>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="nav nav-tabs w-fit-content mb-4">
                        <li class="nav-item">
                            <a class="nav-link type_link @if ($agent->type == 'individual') active @endif " href="#"
                                id="personal-link">{{ ui_change('personal' , 'property_master')  }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link type_link @if ($agent->type == 'company') active @endif " href="#"
                                id="company-link">{{ ui_change('company' , 'property_master')  }}</a>
                        </li>
                    </ul>
                    <div class="col-md-12 agent_form personal-form @if ($agent->type == 'company') d-none @endif "
                        id="personal-form">
                    ...
                    <div class="col-md-12 agent_form @if ($agent->type == 'individual') d-none @endif company-form"
                        id="company-form">
                        <form action="{{ route('agent.update', $agent->id) }}" method="post">
                            @csrf
                            @method('patch')
