...
@section('title')
    {{ ui_change('edit_block_management' , 'property_config') }}
@endsection
@push('css_or_js')
...
@section('content')
    <div class="content container-fluid">
        ...
        <!-- Form -->
        <form class="product-form text-start" action="{{ route('block_management.update' , $main_block->id) }}" method="POST"
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select class="js-select2-custom form-control" id="property_management_id"
                                    ...
                                    @foreach ($property_managements as $property_management)
                                        <option value="{{ $property_management->id }}" {{ ($main_block->property_management_id == $property_management->id) ? 'selected' : ''  }}>{{ $property_management->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-12 col-lg-4 col-xl-12">
                            <div class="form-group">
                                ...
                                <select class="js-select2-custom form-control" id="block_list" name="block_id"
                                    required >
                                    <option value="{{ $main_block->block->id }}"  selected >{{ $main_block->block->name }} </option>
                                    @foreach ($blocks as $block)
                                        <option value="{{ $block->id }}"  {{ ($main_block->block_id == $block->id) ? 'selected' : ''  }}>{{ $block->name }} </option>
                                    @endforeach
                                </select>
                                @error('block_id')
                                    <div class="invalid-feedback">
