...
@section('title', ui_change('all_tenants' , 'property_master') )
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2">
                ...
                {{ ui_change('all_tenants' , 'property_master') }}
                    ...
                    <div class="px-3 py-4">
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                                ...
                                {{-- @can('create_tenant') --}}
                                    <a href="{{ route('tenant.exportTenants') }}" class="btn btn--primary">
                                        <i class="tio-file"></i>
                                        <span class="text">{{ ui_change('export' , 'property_master')  }}</span>
                                    </a>
                                    <a href="{{ route('tenant.create') }}" class="btn btn--primary">
                                        <i class="tio-add"></i>
                                        <span class="text">{{ ui_change('create_tenant' , 'property_master')  }}</span>
                                    </a>
                                {{-- @endcan --}}
                            </div>
                        </div>
                        ...
                        <table id="datatable" style="text-align: {{ Session::get('locale') === 'ar' ? 'right' : 'left' }};"
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th class="text-center">{{ ui_change('tenant_type' , 'property_master')  }}</th>
                            ...
                            <tbody>
                                @foreach ($tenants as $k => $tenant_item)
                                    <tr>
                                        <th scope="row">{{ $tenants->firstItem() + $k }}</th>

                                        <td class="text-center">
                                            {{ ($tenant_item->type == 'individual') ? ($tenant_item->name ?? ui_change('not_available' , 'property_master') ) : ($tenant_item->company_name ?? ui_change('not_available' , 'property_master') ) }}
                                        </td>
                                        <td class="text-center">
                                            {{ $tenant_item->contact_person ?? ui_change('not_available' , 'property_master')}}
                                        </td>
                                        <td class="text-center">
                                            {{ $tenant_item->email1 ?? ui_change('not_available' , 'property_master')}}
                                        </td>

                                        <td class="text-center">
                                            {{ $tenant_item->whatsapp_no ?? ui_change('not_available' , 'property_master')}}
                                        </td>

                                        <td class="text-center">
                                            {{ $tenant_item->type ?? ui_change('not_available' , 'property_master')}}
                                        </td>

                                        <td class="text-center">
                                            {{ $tenant_item->country_master->country->name ?? ui_change('not_available' , 'property_master')}}
                                        </td>

                                        <td class="text-center">
                                            {{-- @can('change_tenant_status') --}}
                                                <form action="{{ route('tenant.status-update') }}" method="post"
                                                    id="subscription_status{{ $tenant_item->id }}_form"
                                                    class="subscription_status_form">
                                                    @csrf
                                                {{-- @endcan --}}
                                                <input type="hidden" name="id"
                                                    value="{{ $tenant_item->id }}">
                                                <label class="switcher mx-auto">
                                                    <input type="checkbox" class="switcher_input"
                                                        id="subscription_status{{ $tenant_item->id }}"
                                                        name="status" value="1"
                                                        {{ $tenant_item->status == 'active' ? 'checked' : '' }}
                                                        onclick="toogleStatusModal(event,'subscription_status{{ $tenant_item->id }}',
                                                'subscription-status-on.png','subscription-status-off.png',
                                                '{{ ui_change('Want_to_Turn_ON' , 'property_master') }} {{ $tenant_item->name }} ',
                                                '{{ ui_change('Want_to_Turn_OFF' , 'property_master') }} {{ $tenant_item->name }} ',
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                {{-- <a class="btn btn-outline-info btn-sm square-btn" title="{{ ui_change('all_tenants' , 'property_master')('barcode') }}"
                                            href="{{ route('tenant.barcode', [$subscription['id']]) }}">
                                            <i class="tio-barcode"></i>
                                        </a> --}}
                                                {{-- <a class="btn btn-outline-info btn-sm square-btn" title="View"
                                                    href="{{ route('tenant.show', [$subscription->id]) }}">
...
@push('script')
    ...
    <script>
        ...
        $('.subscription_status_form').on('submit', function(event) {
            ...
            $.ajax({
                url: "{{ route('tenant.status-update') }}",
                method: 'POST',
                data: $(this).serialize(),
