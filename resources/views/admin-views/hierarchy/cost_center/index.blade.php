...
@endphp
@section('title', __('roles.cost_center_list'))
@push('css_or_js')
                ...
                <div class="col-md-12 mb-3">
                    <div class="card">
                        <div class="card-header">
                            {{ __('roles.add_new_cost_center') }}
                        </div>
                        <div class="card-body" style="text-align: {{ $lang === 'ar' ? 'right' : 'left' }};">
                            <form action="{{ route('cost_center.store') }}" method="post">
                                @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="cost_center_category_id" class="form-control js-select2-custom ">
                                            @foreach ($cost_center_categories as $cost_center_category_item)
                                                <option value="{{ $cost_center_category_item->id }}">
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            placeholder="{{ __('roles.search_by_cost_center_name') }}" aria-label="Search"
                        ...
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        <th>{{ __('general.sl') }}</th>
                                        <th class="text-center">{{ __('roles.cost_center_name') }} </th>
                                        <th class="text-center">{{ __('roles.cost_center_category') }} </th>
                                ...
                                <tbody>
                                    @foreach ($main as $key => $value)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $value->cost_center_category->name }} </td>



                                            <td class="text-center">
                                                <form action="{{ route('cost_center.status-update') }}" method="post"
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-2">
                                                    <a class="btn btn-outline-info btn-sm square-btn"
                                                        ...
                                                        href="{{ route('cost_center.edit', $value->id) }}">
                                                        <i class="tio-edit"></i>
                                                    </a>
...
@push('script')
    ...
    <script>
        $('.product_status_form').on('submit', function(event) {
            ...
            $.ajax({
                url: "{{ route('cost_center.status-update') }}",
                method: 'POST',
                data: $(this).serialize(),
