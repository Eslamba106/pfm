                            ...
                            <table id="datatable"
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('property_reports.total') }}</th>
                                        <th class="text-center">{{ __('property_reports.30_day') }}</th>
                                        <th class="text-center">{{ __('property_reports.30_to_60') }}</th>
                                        <th class="text-center">{{ __('property_reports.60_to_90') }}</th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach ($groupedSummaries as $index => $group)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                {{ $group['tenant']->name ?? $group['tenant']?->company_name }}
                                            </td>
                                            <td class="text-center">{{ number_format($group['totals']['total'], 2) }}</td>
                                            <td class="text-center">{{ number_format($group['totals']['30_day'], 2) }}</td>
                                            <td class="text-center">{{ number_format($group['totals']['30_to_60'], 2) }}</td>
                                            <td class="text-center">{{ number_format($group['totals']['60_to_90'], 2) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                ...
                                <tbody>
                                    @foreach ($totalsByTenant as $tenantInfo)
                                        <tr style="background-color: red; color: white;">
                                            <td colspan="2">
                                                {{ $tenantInfo['tenant']->name ?? $tenantInfo['tenant']->company_name }}
                                            </td>
                                            <td>
                                                الإجمالي: {{ $tenantInfo['total_sum'] }}
                                            </td>
                                        </tr>
                                    @endforeach
                                     ...
                                     @foreach ($invoices as $invoice_item)
                                            ...
                                            <tr >
                                                <th scope="row"><input class="check_bulk_item" name="bulk_ids[]"
                                                        type="checkbox" value="{{ $invoice_item->id }}" />
                                                    {{ $loop->index + 1 }}
                                                </th>
                                                <td class="text-center">
                                                    {{ $invoice_item->tenant->name ?? $invoice_item->tenant->company_name }}
                                                </td>
                                                <td class="text-center">
                                                    {{ $invoice_item->total }}
                                                </td> 
