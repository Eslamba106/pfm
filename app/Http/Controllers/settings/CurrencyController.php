...
class CurrencyController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $currency->save();
        Toastr::success(ui_change('New_Currency_inserted_successfully'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($currency['code'] == 'BDT' && $request->code != 'BDT') {
            ...
            if ($config['status']) {
                Toastr::warning(ui_change('Before_update_BDT').", ".ui_change('disable_the_SSLCOMMERZ_payment_gateway.'));
                return back();
            }
        } elseif ($currency['code'] == 'INR' && $request->code != 'INR') {
            $config = Helpers::get_business_settings('razor_pay');
            if ($config['status']) {
                Toastr::warning(ui_change('Before_update_INR').", ".ui_change('disable_the_RAZOR_PAY_payment_gateway.'));
                return back();
            }
        } elseif ($currency['code'] == 'MYR' && $request->code != 'MYR') {
            $config = Helpers::get_business_settings('senang_pay');
            if ($config['status']) {
                Toastr::warning(ui_change('Before_update_MYR').", ".ui_change('disable_the_SENANG_PAY_payment_gateway.'));
                return back();
            }
        } elseif ($currency['code'] == 'ZAR' && $request->code != 'ZAR') {
            $config = Helpers::get_business_settings('paystack');
            if ($config['status']) {
                Toastr::warning(ui_change('Before_update_ZAR').", ".ui_change('disable_the_PAYSTACK_payment_gateway.'));
                return back();
            }
        }
        $currency->name = $request->name;
        ...
        $currency->exchange_rate = $request->has('exchange_rate') ? $request->exchange_rate : 1;
        $currency->save();
        Toastr::success(ui_change('currency_updated_successfully'));
        return redirect()->back();
    }
    ...
    public function status(Request $request)
    {
        if ($request->status != 1) {
            ...
            if ($count == 1) {
                return response()->json([
                    'status' => 0,
                    'message' => ui_change('You_can_not_disable_all_currencies.')
                ]);
            }
        }
        ...
        return response()->json([
            'status' => 1,
            'message' => ui_change('Currency_status_successfully_changed.')
        ]);
    }
    ...
    public function systemCurrencyUpdate(Request $request)
    {
        ...
        Toastr::success(ui_change('System_Default_currency_updated_successfully'));
        return redirect()->back();
    }
}
