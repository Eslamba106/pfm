...
use App\Services\facility_master\ComplaintCategoryServices;

class ComplaintCategoryController extends Controller
{
    public $complaint_services;
    public $model;
    public function __construct(ComplaintCategoryServices $complaint_services){
    ...
    public function index(Request $request){
        ...
        if ($request->bulk_action_btn === 'update_status'  && is_array($ids) && count($ids)) {
            ...
            (new ComplaintCategory())->setConnection('tenant')->whereIn('id', $ids)->update($data);
            return back()->with('success', __('general.updated_successfully'));
        }
        $search      = $request['search'];
        $query_param = $search ? ['search' => $request['search']] : '';
        $complaint = (new ComplaintCategory())->setConnection('tenant')->when($request['search'], function ($q) use ($request) {
    ...
    public function update(Request $request, $id){
        ...
        try{
            ...
            return redirect()->route('complaint_category.index')->with('success',__('property_master.updated_successfully'));
        }catch(\Exception $e){
            return redirect()->back()->with("error", $e->getMessage());
        }
    }
    public function delete(Request $request){
        ...
        ($complaint_services_delete == true) ? redirect()->route("complaint_category.index")->with("success",__('property_master.deleted_successfully'))
    ...
    public function statusUpdate(Request $request)
    {
        ...
        $main = (new ComplaintCategory())->setConnection('tenant')->findOrFail($request->id);
        $main->update([
